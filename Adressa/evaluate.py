...
def evaluate(model, directory, max_count=sys.maxsize):
    ...
    user_dataset = UserDataset(path.join(directory, config.test_behaviors_file),
                               f'.\\data\\preprocessed_data\\{config.data}\\train\\user2int.tsv')
    ...
    behaviors_dataset = BehaviorsDataset(path.join(f'.\\data\\preprocessed_data\\{config.data}\\test', config.test_behaviors_file))
    ...
    count = 0
    news_df = pd.read_table(path.join(f'./data/preprocessed_data/{config.data}', 'total_news(raw).tsv'),
...
if __name__ == '__main__':
    ...
    model.eval()
    auc, mrr, ndcg5, ndcg10 = evaluate(model, f'./data/preprocessed_data/{config.data}/test')
    print(
        f'AUC: {auc:.4f}\nMRR: {mrr:.4f}\nnDCG@5: {ndcg5:.4f}\nnDCG@10: {ndcg10:.4f}'
    )
